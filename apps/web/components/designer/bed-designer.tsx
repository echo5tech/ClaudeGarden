...
export function BedDesigner({
  ...
  const [activePlant, setActivePlant] = useState<
    CatalogPlant | PlacedPlant | null
  >(null);

  // Hydrate store from server-loaded data on mount
  useEffect(() => {
    store.hydrate(gardenId, initialBedId, initialBed, initialPlaced);
    // Server snapshots change on route navigation; refresh the editor for that bed.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [gardenId, initialBedId]);
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (id.startsWith("palette-")) {
      ...
      place(
        plant,
        (px + delta.x - over.rect.left) / scale,
        (py + delta.y - over.rect.top) / scale,
      );
      ...
      move(
        instanceId,
        p.xInches + delta.x / scale,
        p.yInches + delta.y / scale,
      );
    }
  }
  ...
  async function handleSave() {
    ...
    if (err) {
      ...
      toast.success(
        gardenId.startsWith("demo-")
          ? "Sample bed saved in this browser."
          : "Bed saved!",
      );
      if (!gardenId.startsWith("demo-")) router.refresh();
    }
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <p className="designer-instructions">
        {gardenId.startsWith("demo-")
          ? "Sample bed · Saves stay in this browser."
          : "Your garden plan"}{" "}
        · Drag to arrange. Use + to add with a keyboard. Red outlines need more
        space.
      </p>
      {/* ── Toolbar ── */}
      <div className="flex items-center gap-3 px-4 py-2 border-b bg-white dark:bg-zinc-950 shrink-0 flex-wrap gap-y-2">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <input
            ...
            value={widthFt}
            onChange={(e) =>
              setBed(Math.max(2, +e.target.value) * 12, bed.heightInches)
            }
          ...
          <input
            ...
            value={heightFt}
            onChange={(e) =>
              setBed(bed.widthInches, Math.max(2, +e.target.value) * 12)
            }
            className="w-12 rounded border px-1.5 py-1 text-center text-sm tabular-nums focus:outline-none focus:ring-2 focus:ring-ring"
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="w-11 text-center text-xs text-zinc-500 tabular-nums">
            {zoomPct}%
          </span>
          <Button
            variant="outline"
        ...
        <Button
          ...
          onClick={() => {
            if (
              placed.length &&
              window.confirm(
                "Remove all plants from this draft? Save to apply the change.",
              )
            )
              clear();
          }}
        ...
        {saveError && (
          <span
            role="alert"
            className="text-xs text-red-700 max-w-sm"
            title={saveError}
          >
            {saveError}
          </span>
